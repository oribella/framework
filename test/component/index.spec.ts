...
class Tap extends DefaultGesture {
  ...
  public start(/*evt: Event, pointers: PointerData[]*/): number {
    ...
    // const result = this.listener.start(evt, {}, this.target);
    return 0; // RETURN_FLAG.map(result) + RETURN_FLAG.START_EMITTED;
  }
...
describe.skip('Scenario', () => {
  ...
  beforeEach(() => {
    ...
    instance.registerGesture({ type: 'foo', gesture: Tap });
    instance.activate();
    listener = {
      start: sandbox.spy()
    };
  ...
  it('hepp', () => {
    ...
    instance.on(target, { type: 'foo', listener });
